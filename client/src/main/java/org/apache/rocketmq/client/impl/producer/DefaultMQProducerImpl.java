...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 对生产组名进行校验
                this.checkConfig();
                // 如果生产者组名不为 CLIENT_INNER_PRODUCER 则把生产者实例名设置为进程ID
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    this.defaultMQProducer.changeInstanceNameToPID();
                }
                // 通过 MQ 客户端工厂来创建一个 MQ 客户端实例，如果本地缓存中有则从缓存去取。
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 把生产者注册到 MQ 客户端中，既放入客户端的本地缓存中。
    ...
    private void checkConfig() throws MQClientException {
        // 检查生产组名是否为空、是否组名长度超过 255  、是否与默认的生产组名同名，
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 到这里说明没超时，执行发送消息给 Broker 并获取发送结果。
                        sendResult = this.sendKernelImpl(msg, mq, communicationMode, sendCallback, topicPublishInfo, timeout - costTime);
                        endTimestamp = System.currentTimeMillis();
                        // 更新调用时间到 mqFaultStrategy 里
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                return null;
                            case SYNC:
                                // 如果同步发送失败会启用重试机制，发送消息给另一个 Broker。
     ...
     * 获取 topic 对应的路由
     *
     * 1、先尝试到本地缓存中取，没的话到 Nameserver 中拉取并缓存到本地。
     * 2、如果还是不行的话如果 broker 开启了默认创建 topic 的功能，则会创建一个 TBW102 的 topic 然后返回。
     *
     * @param topic 主题名
     * @return 路由信息
     */
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 先到本地缓存中查找路由信息，没的话则到 NameServer 拉取新的并放入本地缓存中。
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            // 到 NameServer 拉取最新的再获取路由信息
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 还是没有的话创建一个默认的 topic 为 TBW102，前提是 broker 是否开启了默认创建。
    ...
    /**
     * 1、找到要发送的 Broker 地址信息，找不到的话到 Nameserver 拉取新的。
     * 2、对消息进行一些判断和设置，执行 ForbiddenHook 和 SendMessageHook 钩子函数如果有的话。
     * 3、发送消息
     * 4、执行 SendMessageHook 的钩子函数并返回发送结果 SendResult。
     */
    private SendResult sendKernelImpl(
        ...
        // 如果获取不到则到本地缓存找还是没有则到 NameServer 拉取
        if (null == brokerAddr) {
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                // 检查有没有钩子函数 ，有的话则遍历执行 checkForbidden() 方法
                ...
                // 只对 Consumer 有用,如果 topic 是以 %RETRY% 开头则会设置重新消费
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
            ...
            case SEND_OK: {
                try {
                    ...
                    if (null != localTransactionExecuter) {
                        ...
                        // 7、消息发送成功，则由事务监听器来执行本地事务并返回本地事务状态
        ...
        try {
            // 9、向 Broker 发送结束事务的消息（rollback or commit）
    ...
    /**
     * 根据 localTransactionState 来判定是向 Broker 发送 Commit 还是 Rollback 消息。
     */
    public void endTransaction(
        ...
        // 2、消息在 CommitLog 中的 offset
        requestHeader.setCommitLogOffset(id.getOffset());
        // 3、根据本地事务的执行结果设置 commit 或者 rollback
        switch (localTransactionState) {
            // 如果是本地事务提交成功则向 Broker 发送 TRANSACTION_COMMIT_TYPE
            case COMMIT_MESSAGE:
                requestHeader.setCommitOrRollback(MessageSysFlag.TRANSACTION_COMMIT_TYPE);
                break;
            // 如果本地事务回滚了则向 Broker 发送 TRANSACTION_COMMIT_TYPE
            case ROLLBACK_MESSAGE:
